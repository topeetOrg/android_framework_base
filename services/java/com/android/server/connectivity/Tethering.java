...
public class Tethering extends INetworkManagementEventObserver.Stub {
    ...
    public boolean isBluetooth(String iface) {
        synchronized (mPublicSync) {
            for (String regex : mTetherableBluetoothRegexs) {
				 Log.d(TAG, "BT Tether Regexs Init Value:" + regex);    
                if (iface.matches(regex)) return true;
            }
            return false;
        }
	...
	private void startReverseTether(final LinkProperties linkProperties) {
		   ...
		   Thread dhcpThread = new Thread(new Runnable() {
			   public void run() {
				   ...
				   synchronized (mLinkPropertiesLock) {
                       ...
                       Collection<InetAddress> dnses = dhcpResults.linkProperties.getDnses();
					   ...
					   if (dnses != null) {
                            ...
                            if (v4Dnses.size() > 0) {
								...
								for(String dnsip : dnsServers){
                                	...
                                	Log.e(TAG, "dns server list:" + dnsip);
							    }
								
								mPanDnsServers = new String[2];
								mPanDnsServers[0] = dnsServers[0];
								mPanDnsServers[1] = "8.8.8.8";

								//set dns server for bt-pan 
								String iface = "bt-pan";
                				try {
                				    Log.d(TAG, "set Dns Servers For Interface " + iface);				
                				    mNMService.setDnsServersForInterface(iface,mPanDnsServers,"gcn.mediatek.inc");
                					
                				}catch(Exception e){
                                    Log.e(TAG, "Error setDnsServersForInterface", e);
                				}
                
                			    try {
                				    Log.d(TAG, "set Default Interface For Dns " + iface);
                                    mNMService.setDefaultInterfaceForDns(iface);
                				}catch(Exception e){ 
                                    Log.e(TAG, "Error setDefaultInterfaceForDns", e);
                				}
                            }

                       }else {
                          Log.e(TAG, "dns server connection is null");
					   }
					   /*
	  ...
	  private void stopReverseTether() {
		   synchronized (mLinkPropertiesLock) {
    ...
    public void interfaceAdded(String iface) {
        ...
        synchronized (mPublicSync) {
            ...
            mIfaces.put(iface, sm);
            sm.start();
			//Start DHCPCD for network interface "bt-pan" added in kernel
		    if(iface.equals("bt-pan")) {

			    LinkProperties lp = new LinkProperties();
                lp.setInterfaceName(iface);
			
				try {
					Log.d(TAG, "run dhcpcd daemon " + iface);
					startReverseTether(lp);
				}catch (Exception e){
				    Log.e(TAG, "Error startReverseTether of dhcpcd daemon", e);
					return;
				}
    ...
    public void interfaceRemoved(String iface) {
		...
		//Stop DHCPCD for network interface "bt-pan" added in kernel
		if(iface.equals("bt-pan")){
			try{
				stopReverseTether();
			}catch (Exception e){
			    Log.e(TAG, "Error clear bt-pan dhcp", e);
			 	//return;
			}
		}
        synchronized (mPublicSync) {
